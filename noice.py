...
@bot.on(admin_cmd(pattern="insut", outgoing=True)) 
async def cry(e):
    """ y u du dis, i cry everytime !! """
    if not e.text[0].isalpha() and e.text[0] not in ("/", "#", "@", "!"):
...
@bot.on(admin_cmd(pattern="izal(?: |$)(.*)", outgoing=True)) 
async def izal(zgfy):
...
@bot.on(admin_cmd(pattern="irand", outgoing=True))
...
@bot.on(admin_cmd(pattern="roon", outgoing=True)) 
...
@bot.on(admin_cmd(pattern="disable roon", outgoing=True)) 
...
@bot.on(admin_cmd(pattern="enable roon", outgoing=True)) 
...
@bot.on(admin_cmd(pattern="lgfy (.*)", outgoing=True))
