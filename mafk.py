...
async def set_not_afk(event):
    ...
    if ".mafk" not in current_message and "yes" in USER_AFK:  # pylint:disable=E0602
        ...
        try:
            ...
            await bot.send_message(  # pylint:disable=E0602 # Originally by @ProgrammingError
...
@bot.on(admin_cmd(pattern=r"mafk (.*)", outgoing=True))  # pylint:disable=E0602
async def _(event):
    if event.fwd_from:
        return
...
# I think its first for DARKultra
        ...
        if reason:
            ...
            await bot.send_message(event.chat_id, f"**I am Going afk!**", file=pic)
